<section class="box-cta-2 background-body overflow-hidden">
				<div class="bg-shape top-50 start-50 translate-middle"></div>
				<div class="container position-relative z-1">
					<div class="row ">
						<div class="col-lg-5 pe-lg-5 ">
							<h3 class="text-white wow fadeInDown">Want to Calculate Your Car Payment?</h3>
							<p class="text-lg-medium text-white wow fadeInUp">Match with up to 4 lenders to get the lowest
								rate available with no markups, no fees, and no obligations.</p>
						</div>
						<div class="col-lg-6 offset-lg-1">
							<div class="mb-30 background-card p-md-5 p-4 rounded-3 mt-lg-0 mt-30 wow fadeIn">
								<h5 class="neutral-1000 mb-2">Car Loan Calculator</h5>
								<p class="text-sm-medium neutral-500 mb-25">Estimate your monthly auto loan payments with
									this calculator.</p>
								<div class="form-contact">
									<div class="row">
										<div class="col-lg-6">
											<div class="form-group">
												<label class="text-sm-medium neutral-1000">Price of vehicle</label>
												<input class="form-control" type="text" placeholder="$20,000">
											</div>
										</div>
										<div class="col-lg-6">
											<div class="form-group">
												<label class="text-sm-medium neutral-1000">Interest rate</label>
												<input class="form-control" type="text" placeholder="5%">
											</div>
										</div>
										<div class="col-lg-6">
											<div class="form-group">
												<label class="text-sm-medium neutral-1000">Terms</label>
												<input class="form-control" type="text" placeholder="12 months">
											</div>
										</div>
										<div class="col-lg-6">
											<div class="form-group">
												<label class="text-sm-medium neutral-1000">Down payment</label>
												<input class="form-control" type="text" placeholder="$12,000">
											</div>
										</div>
										<div class="row py-4">
											<div class="col-md-5 col-8 d-flex flex-column gap-1">
												<p class="text-sm-bold neutral-1000">Down payment ammout</p>
												<p class="text-sm-bold neutral-1000">Amount financed</p>
												<p class="text-sm-bold neutral-1000">Monthly payment</p>
											</div>
											<div class="col-md-7 col-4 d-flex flex-column gap-1 align-items-end align-items-md-start">
												<p class="text-sm-bold neutral-1000">$12,000</p>
												<p class="text-sm-bold neutral-1000">$800,00</p>
												<p class="text-sm-bold text-primary-dark">$480,00</p>
											</div>
										</div>
										<div class="col-lg-12">
											<button class="btn btn-book">
												Apply for a loan
												<i class="fi fi-rr-arrow-right ms-2"></i>
											</button>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
					<div class="row align-items-center">
						<div class="border-top py-3 mt-3"></div>
						<div class="col-lg-7 mb-20 wow fadeIn">
							<div class="row">
								<div class="col-md-3 col-6 mb-md-0 mb-4 d-flex flex-column align-items-center align-items-md-start">
									<div class="d-flex justify-content-center justify-content-md-start">
										<h3 class="count text-white" data-count="45">0</h3>
										<h3 class="text-white">+</h3>
									</div>
									<div class="position-relative">
										<p class="text-lg-bold text-white">Global</p>
										<p class="text-lg-bold text-white">Branches</p>
									</div>
								</div>
								<div class="col-md-3 col-6 mb-md-0 mb-4 d-flex flex-column align-items-center align-items-md-start">
									<div class="d-flex justify-content-center justify-content-md-start">
										<h3 class="count text-white" data-count="29">0</h3>
										<h3 class="text-white">K</h3>
									</div>
									<div class="position-relative">
										<p class="text-lg-bold text-white">Destinations</p>
										<p class="text-lg-bold text-white">Collaboration</p>
									</div>
								</div>
								<div class="col-md-3 col-6 mb-md-0 mb-4 d-flex flex-column align-items-center align-items-md-start">
									<div class="d-flex justify-content-center justify-content-md-start">
										<h3 class="count text-white" data-count="20">0</h3>
										<h3 class="text-white">+</h3>
									</div>
									<div class="position-relative">
										<p class="text-lg-bold text-white">Years</p>
										<p class="text-lg-bold text-white">Experience</p>
									</div>
								</div>
								<div class="col-md-3 col-6 mb-md-0 mb-4 d-flex flex-column align-items-center align-items-md-start">
									<div class="d-flex justify-content-center justify-content-md-start">
										<h3 class="count text-white" data-count="168">0</h3>
										<h3 class="text-white">K</h3>
									</div>
									<div class="position-relative">
										<p class="text-lg-bold text-white">Happy</p>
										<p class="text-lg-bold text-white">Customers</p>
									</div>
								</div>
							</div>
						</div>
						<div class="col-lg-4 offset-lg-1 wow fadeIn">
							<div class="box-authors-partner background-body wow fadeInUp p-4">
								<div class="authors-partner-left">
									<img src="/assets/imgs/page/homepage5/author.png" alt="Carento"><img src="/assets/imgs/page/homepage5/author2.png" alt="Carento"><img src="/assets/imgs/page/homepage5/author3.png" alt="Carento">
									<span class="item-author">
										<i class="fi fi-rr-plus text-dark"></i>
									</span>
								</div>
								<div class="authors-partner-right">
									<p class="text-sm neutral-1000">1684 people used <strong>Carento </strong>in the last
										<strong>24 hours</strong></p>
								</div>
							</div>
						</div>
					</div>
				</div>
			</section>
