<div class="top-bar top-bar-2 top-bar-3 {{ $transparent ? 'bg-transparent' : '' }}">
    <div class="container-fluid">
        <div class="text-header-info d-flex align-items-center gap-3">
            <a class="phone-head d-inline-flex align-items-center" href="tel:{{ $phoneRaw }}">
                <i class="fi fi-rr-phone-call me-2"></i>
                <span class="d-none d-lg-inline-block">{{ $phone }}</span>
            </a>
            <a class="email-head d-inline-flex align-items-center" href="mailto:{{ $email }}">
                <i class="fi fi-rr-envelope me-2"></i>
                <span class="d-none d-lg-inline-block">{{ $email }}</span>
            </a>
        </div>
        <div class="text-header topbar-announcement-ticker" style="position: relative; overflow: hidden; min-height: 38px; display: flex; align-items: center; justify-content: center;">
            @if(count($announcements) > 0)
                @foreach($announcements as $index => $item)
                    <div class="announcement-item d-flex align-items-center gap-2" style="transition: opacity 0.4s ease-in-out; {{ $index > 0 ? 'display: none !important; opacity: 0;' : 'opacity: 1;' }}">
                        <div class="text-unlock text-sm-medium text-white d-flex align-items-center"><i class="fi fi-rr-car me-2"></i><span>{{ $item['text'] }}</span></div>
                        @if(!empty($item['button_text']))
                            <a class="btn btn-brand-2 btn-small text-dark px-3 py-2 text-xs-medium d-inline-flex align-items-center justify-content-center" href="{{ $item['button_url'] ?? '#' }}">
            ...
            @else
                ...
                <a class="btn btn-brand-2 btn-small text-dark px-3 py-2 text-xs-medium ms-2 d-inline-flex align-items-center justify-content-center" href="/cars-list-1">
...
document.addEventListener('DOMContentLoaded', function () {
    var tickers = document.querySelectorAll('.topbar-announcement-ticker');
    tickers.forEach(function (ticker) {
        if (ticker.dataset.tickerInitialized) return;
        ticker.dataset.tickerInitialized = 'true';
        var items = ticker.querySelectorAll('.announcement-item');
        ...
        setInterval(function () {
            ...
            setTimeout(function () {
                ...
                nextItem.style.setProperty('display', 'flex', 'important');
                setTimeout(function () {
