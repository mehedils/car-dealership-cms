...
@section('content')
@php
    $aboutBg = setting('about_hero_bg_image');
    if ($aboutBg) {
        $aboutBgUrl = (str_starts_with($aboutBg, 'http') || str_starts_with($aboutBg, '/')) ? $aboutBg : asset('storage/' . $aboutBg);
    } else {
        $aboutBgUrl = asset('assets/imgs/page-header/banner4.png');
    }

    $aboutStoryImg = setting('about_story_image');
    if ($aboutStoryImg) {
        $aboutStoryImgUrl = (str_starts_with($aboutStoryImg, 'http') || str_starts_with($aboutStoryImg, '/')) ? $aboutStoryImg : asset('assets/imgs/page/homepage1/author2.png');
    } else {
        $aboutStoryImgUrl = asset('assets/imgs/page/homepage1/author2.png');
    }
@endphp
<div>
    {{-- Page Header --}}
    <div class="page-header pt-30 background-body">
        <div class="custom-container position-relative mx-auto">
            <div class="bg-overlay rounded-12 overflow-hidden">
                <img class="w-100 h-100 rounded-12 img-banner" src="{{ $aboutBgUrl }}" alt="{{ setting('about_hero_title', __('About Our Dealership')) }}" />
            </div>
            <div class="container position-absolute z-1 top-50 start-50 translate-middle text-center">
                <h2 class="text-white">{{ __(setting('about_hero_title', 'About Our Dealership')) }}</h2>
                <p class="text-lg-medium text-white opacity-75">{{ __(setting('about_hero_subtitle', 'Your Trusted Partner in Premium Automobile Sales & Service')) }}</p>
            </div>
        </div>
    </div>
            ...
            <div class="row align-items-center">
                <div class="col-lg-6 mb-30">
                    <div class="position-relative rounded-16 overflow-hidden shadow-sm">
                        <img class="w-100 rounded-16" src="{{ $aboutStoryImgUrl }}" alt="{{ setting('about_story_title', 'Dedicated to Excellence in Automotive Solutions') }}" style="max-height: 450px; object-fit: cover;">
                    </div>
                </div>
                <div class="col-lg-6 mb-30 ps-lg-5">
                    <span class="text-sm-bold text-uppercase neutral-500 tracking-wide">{{ __(setting('about_story_badge', 'Who We Are')) }}</span>
                    <h3 class="neutral-1000 mt-10 mb-20">{{ __(setting('about_story_title', 'Dedicated to Excellence in Automotive Solutions')) }}</h3>
                    @if(setting('about_story_description'))
                        <div class="text-md-regular neutral-500 mb-30">
                            {!! nl2br(e(setting('about_story_description'))) !!}
                        </div>
                    @else
                    ...
                    <button class="btn btn-brand-2 py-3 px-4 text-md-bold d-inline-flex align-items-center justify-content-center" data-bs-toggle="modal" data-bs-target="#leadModal" style="gap: 8px;">
                        ...
                        <span style="line-height: 1;">{{ __('Contact Our Team') }}</span>
                    </button>
                </div>
            </div>
        </div>
